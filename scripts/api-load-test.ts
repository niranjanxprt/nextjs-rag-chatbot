...
class APILoadTester {
  ...
  private async testEndpoint(
    ...
    try {
      ...
      console.log(`✅ Worker ${workerId}: ${method} ${endpoint} - ${response.status} (${duration.toFixed(2)}ms)`)
      ...
      console.log(`❌ Worker ${workerId}: ${method} ${endpoint} - ERROR: ${errorMessage} (${duration.toFixed(2)}ms)`)
    } finally {
      this.activeRequests--
    }
  }
