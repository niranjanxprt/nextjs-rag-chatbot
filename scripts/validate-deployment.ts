...
class ValidationRunner {
  ...
  async runCommand(command: string, description: string): Promise<boolean> {
    try {
      ...
      Logger.error(`${description} failed: ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
...
class PreDeploymentValidator {
  ...
  async validateDependencies(): Promise<boolean> {
    ...
    const tsExists = await fs.access('node_modules/typescript').then(() => true).catch(() => false)
    if (!tsExists) {
  ...
  async validateExternalServices(): Promise<boolean> {
    ...
    for (const service of services) {
      try {
        ...
        Logger.error(`${service.name} connection error: ${error instanceof Error ? error.message : String(error)}`)
        allConnected = false
      }
    }
