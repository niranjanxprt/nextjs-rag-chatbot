  ...
  const uploadFile = useCallback(
    async (uploadFile: UploadFile): Promise<void> => {
      try {
        ...
        setFiles(prev =>
          prev.map(f =>
            f.id === uploadFile.id ? { ...f, status: 'processing', progress: 50, documentId } : f
          )
        )
        ...
        toast.success(
          ...
          `${uploadFile.file.name} has been processed and is ready for search.`
        )
      } catch (error) {
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload PDF, TXT, Markdown, or Word documents to add to your knowledge base. Maximum file
            size: 10MB. Maximum {MAX_FILES} files at once.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div
