...
jest.mock('@/lib/services/cache', () => ({
  ...
  set: jest.fn(),
  del: jest.fn(),
  getStats: jest.fn(() => Promise.resolve({ hits: 0, misses: 0, sets: 0, deletes: 0, errors: 0, memorySize: 0 }))
}))
...
describe('Stuck Instance Detection', () => {
  ...
  test('should detect infinite loop in stream processing', async () => {
  ...
  test('should detect promise rejection handling', async () => {
